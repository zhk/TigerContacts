...
Cactus::String CMyChiToLetter::GetLetter(const Cactus::String& strText)
{
	unsigned char ucHigh, ucLow;
	int nCode;
	Cactus::String strValue = "";
	...
	for (size_t i = 0; i < strText.size(); i++)
	{
		...
		Cactus::String strRes = FindLetter(nCode);
		if(!_bFullPinYin && strRes.size() > 0)
		{
		...
		if(!m_blnAllBiG)
		{
			for(size_t j = 1; j < strRes.size(); j++)
			{
...
Cactus::String CMyChiToLetter::FindLetter(int nCode)
{
	Cactus::String strValue;
