...
PEXCEPTION_POINTERS pExceptionInfo )
{
    ...
    _stprintf(crash_folder_path, _T("%s\\%s"), module_folder_name, CrashFolder);
    ...
    _stprintf(m_szLogFileName, _T("%s\\%s_[%4u-%02u-%02u_%02u-%02u-%02u].txt"),
	...
	{
		TCHAR szMDFile[MAX_PATH];
		_tcscpy( szMDFile, m_szLogFileName );
		//strcat(szMDFile, ".mdmp");
		_stprintf( _tcsrchr( szMDFile, '.' ) + 1, _T("mdmp") );

		// If file is exist, delete it;
		if ( SetFileAttributes( szMDFile, FILE_ATTRIBUTE_NORMAL ) )
			DeleteFile( szMDFile );

		HANDLE hFile = CreateFile( szMDFile, GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL );
...
PVOID pAddress )
{
    ...
    else if ( length == 4 )
    {
        ...
        {
            if ( !IsBadStringPtr( *(LPCTSTR*)pAddress, 32) )
            {
